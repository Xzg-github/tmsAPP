...
const URL_RECEIVE = '/api/bill/receive_monthly_bill/settlement';//获取结算单位
const URL_JOIN = '/api/bill/receive_monthly_bill/addSettlement';//加入结算单位
...
const onOkActionCreator = () => async(dispatch,getState) =>{
  const {tableItems,receivableMonthBillId,searchData} = getSelfState(getState());
  const items = tableItems.filter(item => item.checked);
  const body = {
    currency:searchData.currency,
    customerId:searchData.customerId,
    receivableMonthBillId,
...
export default async(search = {},config,receivableMonthBillId) => {
  const Container = connect(mapStateToProps, actionCreators)(AddDialog);
  const list = {data:[]};
  let othrer = {search,receivableMonthBillId};
