...
const sortActionCreator = async (dispatch, getState) => {
  ...
  const newFilters = await showFilterSortDialog(filters, 'receive_change_sort');
...
const templateManagerActionCreator = async (dispatch, getState) => {
  ...
  if(true === await showTemplateManagerDialog(tableCols, 'receive_change_export')) {
    dispatch(action.assign({buttons: helper.setExportBtns('receive_change_export', buttons, tableCols)}));
  }
};
