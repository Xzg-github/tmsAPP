...
const sortActionCreator = async (dispatch, getState) => {
  const {filters} = getSelfState(getState());
  const newFilters = await showFilterSortDialog(filters, 'receive_bill_sort');
...
const templateManagerActionCreator = async (dispatch, getState) => {
  ...
  if(true === await showTemplateManagerDialog(tableCols, 'receive_bill_export')) {
    dispatch(action.assign({buttons: helper.setExportBtns('receive_bill_export', buttons, tableCols)}));
  }
};
