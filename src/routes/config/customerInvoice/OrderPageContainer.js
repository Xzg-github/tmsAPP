...
const STATE_PATH = ['customerInvoice'];
const action = new Action(STATE_PATH);

const initActionCreator = () => async (dispatch) => {
  dispatch(action.assign({status: 'loading'}));
  const urlConfig = '/api/config/customer_invoice/config';
  const urlList = '/api/config/customer_invoice/list';
...
const delActionCreator = async (dispatch, getState) => {
  ...
  const url = `/api/config/customer_invoice`;
