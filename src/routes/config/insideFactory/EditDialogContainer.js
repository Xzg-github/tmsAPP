...
const URL_SAVE = '/api/config/inside_factory/add';
const URL_DISTRICT_OPTIONS = '/api/config/inside_factory/district_options';
const URL_CHARGE_PLACE_OPTIONS = '/api/config/inside_factory/charge_place_options';
...
const createContainer = (statePath, afterEditActionCreator) => {
  ...
  const okActionCreator = () => async (dispatch, getState) => {
    ...
    //添加固定值customerId = -1
    const reqBody = {customerId : -1, ...body};
    const option = postOption(reqBody);
...
const STATE_PATH = ['config', 'insideFactory', 'edit'];
