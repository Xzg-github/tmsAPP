...
const URL_DELETE = '/api/config/supplierPriceDetail/freightDelete';
const URL_ABLE = '/api/config/supplierPriceDetail/freightAble';
...
const batchActionCreator = async (dispatch, getState) => {
  ...
  const supplierPriceIds = Array.from(new Set(checkList.map(o => o.supplierPriceId)));
  if (supplierPriceIds.length > 1) return showError('请勾选同一个报价合同下的数据！');
  const {supplierPriceId, supplierPriceCode, supplierId} = checkList[0];
  const defaultValue = {
    supplierPriceId,
    supplierPriceCode,
    supplierId,
...
const configActionCreator = async (dispatch, getState) => {
  ...
  showColsSetting(tableCols, okFunc, 'supplierPriceDetail_freight_tablecols');
};
