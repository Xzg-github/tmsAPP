...
import OrderPage from '../tenantRuleTypes/OrderPage/OrderPage';
...
const URL_DEL = '/api/platform/jurisdiction/dataType/del';
...
const searchAction = async (dispatch, getState) => {
  ...
  const {result} = await fetchJson(URL_SEARCH, postOption(searchData));
...
const editAction = (dispatch, getState) => {
  ...
  let newArray = [];
  const arr = deepCopy(tableItems[index]);
  arr.relationList.map((item) => {
    newArray.push(item.value)
  });

  arr.relationList = newArray;
  const payload = buildEditDialogState(editConfig, arr, true);
...
const doubleClickActionCreator = (rowIndex) => (dispatch, getState) => {
  const {editConfig, tableItems} = getSelfState(getState());
  let newArray = [];
  const arr = deepCopy(tableItems[rowIndex]);
  arr.relationList.map((item) => {
    newArray.push(item.value)
  });
...
const delAction = async (dispatch, getState) => {
  ...
  const {returnCode, result, returnMsg} = await fetchJson(`${URL_DEL}/${item.id}`, 'delete');
  if (returnCode === 0) {
