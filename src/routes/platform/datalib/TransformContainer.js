...
import {buildAddState} from '../datalib/TransformAddDialogContainer';
import {EnhanceEditDialog} from '../../../components/Enhance';
import TransformAddDialogContainer from './TransformAddDialogContainer';
import {search2} from '../../../common/search';
import {toFormValue} from '../../../common/check';

const STATE_PATH = ['config', 'datalib', 'transform'];
const action = new Action(STATE_PATH);
const URL_TRANS_LIST = '/api/config/datalib/trans_list';
const URL_TRANS_DEL = '/api/config/datalib/trans_del';
...
const updateTable = async (dispatch, getState) => {
  ...
  return search2(dispatch, action, URL_TRANS_LIST, currentPage, pageSize, toFormValue(searchDataBak));
};

const pageNumberActionCreator = (currentPage) => (dispatch, getState) => {
  const {pageSize, searchDataBak={}} = getSelfState(getState());
  const newState = {currentPage};
  return search2(dispatch, action, URL_TRANS_LIST, currentPage, pageSize, toFormValue(searchDataBak), newState);
};
...
const searchClickActionCreator = async (dispatch, getState) => {
  ...
  return search2(dispatch, action, URL_TRANS_LIST, 1, pageSize, toFormValue(searchData), newState);
};
...
const delActionCreator = async(dispatch, getState) => {
  ...
  const json = await fetchJson(`${URL_TRANS_DEL}?id=${item.id}`, 'delete');
  if (json.returnCode) {
...
const Component = EnhanceEditDialog(OrderPage, TransformAddDialogContainer);
