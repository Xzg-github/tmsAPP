...
const URL_DEL = `${service}/serviceResource/delete`;
...
api.post('/list', async (req, res) => {
  const url = `${service}/serviceResource/listBySearch`;
  res.send(await fetchJsonByNode(req, url, postOption(searchAdapter(req.body))));
});

// 新增
api.post('/new', async (req, res) => {
  const url = `${service}/serviceResource/insert`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});

// 编辑
api.post('/update', async (req, res) => {
  const url = `${service}/serviceResource/update`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});
