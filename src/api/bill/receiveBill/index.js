...
api.post('/cunstomer_contacts', async(req, res) => {
  const url = `${archiver_service}/customer_contact/${req.body.customerId}/drop_list_info/enabled_type_enabled`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});
...
api.post('/joinDetail/:id', async (req, res) => {
  const url = `${service}/receivable_bill_charge/batch/${req.params.id}`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});
...
api.post('/save', async (req, res) => {
  const url = `${service}/receivable_bill`;
