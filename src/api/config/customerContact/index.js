...
api.post('/list', async (req, res) => {
  const url = `${service}/customer_contact/list/search`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});

//新增 编辑
api.post('/add', async (req, res) => {
  const url = `${service}/customer_contact`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});

//启用与禁用
api.put('/enable/:enabledType', async (req, res) => {
  const url = `${service}/customer_contact/batch/${req.params.enabledType}`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body, 'put')));
});

//删除
api.delete('/delete', async (req, res) => {
  const url = `${service}/customer_contact/batch`;
