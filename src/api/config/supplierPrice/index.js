...
api.post('/list', async (req, res) => {
  const url = `${archiver_service}/supplier_price/list/search`;
...
api.post('/delete', async (req, res) => {
  const url = `${archiver_service}/supplier_price/batch/delete`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body, 'delete')));
});

// 启用/禁用
api.post('/able', async (req, res) => {
  const url = `${archiver_service}/supplier_price/batch/${req.body.enabledType}`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body.ids, 'put')));
});

// 获取编辑界面合同信息数据
api.get('/detail/:id', async (req, res) => {
  const url = `${archiver_service}/supplier_price/search/${req.params.id}`;
  res.send(await fetchJsonByNode(req, url));
});

// 新增合同信息保存
api.post('/contractAdd', async (req, res) => {
  const url = `${archiver_service}/supplier_price/add`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});

// 编辑合同信息保存
api.post('/contractSave', async (req, res) => {
  const url = `${archiver_service}/supplier_price/save`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body, 'put')));
});

// 编辑合同信息提交
api.post('/contractCommit', async (req, res) => {
  const url = `${archiver_service}/supplier_price/submit`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body, 'put')));
});
