...
api.post('/contract', async (req, res) => {
  const {maxNumber, filter, customerId} = req.body;
  const url = `${archiver_service}/customer_price/drop_list/${customerId}`;
  const {returnCode, result=[], returnMsg} = await fetchJsonByNode(req, url);
  const options = result.filter(o => o.customerPriceCode.includes(filter)).slice(0, maxNumber).map(o => ({
    title: o.customerPriceCode, value: o.id
  }));
...
// 获取运费列表数据
api.post('/freightList', async (req, res) => {
...
// 获取附加费列表数据
api.post('/extraChargeList', async (req, res) => {
