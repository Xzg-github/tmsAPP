...
api.post('/list', async (req, res) => {
  const url = `${archiver_service}/supplier/list/search`;
  const list = await fetchJsonByNode(req, url, postOption(req.body));
  list.result.data.forEach(obj => obj.isContract = arr[obj.isContract]);
  res.send(list);
});

// 获取采购人员
api.post('/buyers', async (req, res) => {
...
api.post('/add', async (req, res) => {
  const url = `${archiver_service}/supplier`;
  res.send(await fetchJsonByNode(req, url, postOption(req.body)));
});
